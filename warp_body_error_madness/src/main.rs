...
pub fn extract_request_data_filter_body_stream() -> impl Filter<Extract=(FullPath, impl Stream<Item=Result<impl Buf, warp::Error>>), Error=warp::Rejection> + Clone {
    warp::path::full()
...
pub fn proxy_routes() -> impl Filter<Extract=(impl warp::Reply, ), Error=warp::Rejection> + Clone {
    warp::any()
        ...
        .and_then(|fullpath, body| {
            execute_forward_request(body)
        })
}

pub async fn execute_forward_request(body: impl Stream<Item=Result<impl Buf, warp::Error>> + Send + 'static) -> std::result::Result<impl Reply, Rejection> {
    let body = body
